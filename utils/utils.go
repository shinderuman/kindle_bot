...
func SearchItems(client paapi5.Client, title string) (*entity.Response, error) {
...
func AlertToSlack(err error) error {
	log.Println(err)
...
func UpdateGist(ASINs []KindleBook) error {
	url := fmt.Sprintf("https://api.github.com/gists/%s", EnvConfig.GistID)

	var content []string
	for _, i := range ASINs {
		content = append(content, fmt.Sprintf("* [[%s]%s](%s)", i.ReleaseDate.Format("2006-01-02"), i.Title, i.URL))
	}
